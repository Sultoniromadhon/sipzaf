...
@section('content')
<div class="flex justify-center items-center h-screen bg-gray-100 px-6 bg-gradient-to-r from-sky-500 to-indigo-500">
    <div class="p-6 max-w-sm w-full bg-white shadow-md rounded-md">
        <div class="flex justify-center items-center">
            <span class="text-gray-700 font-semibold text-3xl">LOGIN | SIPZAF</span>
        </div>
        @if (session('status'))
        ...
        <form class="mt-4" action="{{ route('login') }}" method="POST">
            ...
            <label class="block">
                ...
                <input type="email" name="email" value="{{ old('email') }}"
                    class="form-input mt-1 block w-full rounded-md focus:outline-none" placeholder="Email">
                @error('email')
